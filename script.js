(() => {
  // Polyfill: String.prototype.replaceAll (for older Safari)
...
alreadyRated: { ar: "تم استلام تقييمك مسبقًا.", en: "You already rated.", he: "כבר דירגת." },
  ...
  // Language switch loading UI (bottom)
  const langLoadingEl = document.getElementById("langLoading");
  const langLoadingText = document.getElementById("langLoadingText");
  const langLoadingDict = {
    ar: "جارٍ تطبيق اللغة…",
    en: "Applying language…",
    he: "מיישם שפה…"
  };
  let langLoadingTimer = null;
  function showLangLoading(lang){
    if(!langLoadingEl) return;
    if(langLoadingText) langLoadingText.textContent = langLoadingDict[lang] || langLoadingDict.ar;
    langLoadingEl.classList.add("is-show");
    langLoadingEl.setAttribute("aria-hidden", "false");
    if(langLoadingTimer) clearTimeout(langLoadingTimer);
  }
  function hideLangLoading(){
    if(!langLoadingEl) return;
    langLoadingTimer = setTimeout(()=>{
      langLoadingEl.classList.remove("is-show");
      langLoadingEl.setAttribute("aria-hidden", "true");
    }, 450);
  }
...
const setLang = (lang) => {
    ...
    document.querySelectorAll("[data-i18n]").forEach(el => {
      ...
      let val = dict[key] && dict[key][lang];
      if (!val && dict[key] && dict[key].ar) val = dict[key].ar;
  ...
  document.querySelectorAll(".langchip").forEach(btn => {
    btn.addEventListener("click", (e) => {
      ...
      // Apply on next frame for smoother UI on Safari
...
(() => {
  ...
  document.querySelectorAll("[data-scroll]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      e.preventDefault();
      // Prevent mobile focus-scroll jumps
      ...
      const sticky = document.querySelector(".langbar");
      const stickyH = sticky ? sticky.getBoundingClientRect().height : 0;
      // Scroll so the *section title* starts right under the sticky lang bar
      const top = el.getBoundingClientRect().top + window.scrollY - stickyH - 10;
      window.scrollTo({ top, behavior: "smooth" });
      // iOS/Safari: a second align pass prevents 'jump to middle'
      setTimeout(() => {
        const top2 = el.getBoundingClientRect().top + window.scrollY - stickyH - 10;
