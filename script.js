...
alreadyRated: { ar: "تم استلام تقييمك مسبقًا.", en: "You already rated.", he: "כבר דירגת." },
    ...
    loadingApplying: { ar: "جارٍ تطبيق اللغة…", en: "Applying language…", he: "מיישם שפה…" },
  ...
  function renderReviews(lang, reset){
    ...
    let nextCount;
    if(reset){
      nextCount = Math.min(all.length, REVIEWS_INITIAL);
    } else {
      nextCount = Math.min(all.length, reviewsShown + REVIEWS_STEP);
    }
  ...
  function updateReviewsMeta(){
    ...
    let sum = 0;
    all.forEach(r => { sum += (Number(r.stars) || 5); });
    const avg = count ? (sum / count) : 0;
    countEl.textContent = String(count);
    avgEl.textContent = avg ? avg.toFixed(1) : "0.0";
  }

  // Fancy language loading (never blocks clicks)
  let langLoadingTimer = null;
  let langSwitchLock = false;

  function showLangLoading(){
    const el = document.getElementById("langLoading");
    if(!el) return;
    el.hidden = false;
  ...
  function hideLangLoading(){
    ...
    clearTimeout(langLoadingTimer);
    langLoadingTimer = setTimeout(() => { el.hidden = true; }, 260);
  }

  // Helper: scroll so the pressed button starts at the top (below sticky langbar)
  function scrollToButtonTop(btn){
    if(!btn) return;
    const sticky = document.querySelector(".langbar");
    const stickyH = sticky ? sticky.getBoundingClientRect().height : 0;
    const top = btn.getBoundingClientRect().top + window.scrollY - stickyH - 10;
  ...
  const setLangWithLoading = (lang) => {
    if(langSwitchLock) return;
  ...
  document.querySelectorAll(".langchip").forEach(btn => {
    ...
    btn.addEventListener("click", (e) => {
      e.preventDefault();
      e.stopPropagation();
      btn.blur(); // prevent mobile focus jump
      setLangWithLoading(btn.dataset.lang);
...
(() => {
  ...
  document.querySelectorAll("[data-scroll]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      // After opening, keep the pressed button aligned to the top (no weird jumps)
      btn.blur();
      requestAnimationFrame(() => {
        // scroll to the button itself (as requested)
        if (typeof scrollToButtonTop === "function") {
          scrollToButtonTop(btn);
        } else {
