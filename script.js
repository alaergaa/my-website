(() => {
  ...
  const dict = {
    ...
    secAboutText: { ar: "مركز الارتقاء الطبي هو كيان متخصص في الحلول الطبية الآمنة والمبتكرة. نهدف لتقديم منتجات تعيد لك الثقة بنسبة 100% وبدون أي آثار جانبية.", en: "Medical Elevation Center specializes in safe, innovative medical solutions.", he: "מרכז העלייה הרפואי מתמחה בפתרונות רפואיים בטוחים וחדשניים." },
...
(() => {
  ...
  // **تعديل 1: جعل "فوائد المنتج" (#benefits) هو القسم المفتوح افتراضياً بدلاً من الطلب**
  ...
  // **تعديل 2: تفعيل زر "اطلب الآن" العائم الجديد ليفتح قسم الطلب**
  document.querySelectorAll("[data-scroll]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      // تغيير حالة الأزرار العلوية فقط إذا لم يكن الزر المضغوط هو الزر العائم
      if (btn.classList.contains('aBtn')) {
        document.querySelectorAll(".aBtn").forEach((b) => b.classList.remove("is-active"));
        btn.classList.add("is-active");
      } else {
        // إذا كان الزر العائم، نقوم بتفعيل زر الطلب الأساسي في الأعلى
        document.querySelectorAll(".aBtn").forEach((b) => b.classList.remove("is-active"));
        const topOrderBtn = document.querySelector(`.aBtn[data-scroll="${sel}"]`);
        if(topOrderBtn) topOrderBtn.classList.add("is-active");
      }
