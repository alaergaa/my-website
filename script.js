...
function scrollToButtonTop(btn){
  ...
  const top = btn.getBoundingClientRect().top + window.scrollY - stickyH - 12;
...
const setLangWithLoading = (lang) => {
    ...
    // Apply language on next frame for smoothness
    requestAnimationFrame(() => {
      try{
        setLang(lang);
      } finally {
        // keep loading briefly so it feels intentional
        clearTimeout(langLoadingTimer);
  ...
  document.querySelectorAll(".langchip").forEach(btn => {
    btn.addEventListener("click", (e) => { e.preventDefault(); setLangWithLoading(btn.dataset.lang); });
  });
