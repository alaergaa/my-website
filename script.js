...
(() => {
  ...
  const paint = (v) => {
    stars.forEach(b => { b.classList.remove("is-half"); b.classList.toggle("is-on", Number(b.dataset.v) <= v); });
  };
  ...
  const paintAvg = (avg) => {
    ...
    stars.forEach(b => {
      ...
      if (n <= full) b.classList.add("is-on");
      else if (half && n === full + 1) b.classList.add("is-half");
    });
  };
