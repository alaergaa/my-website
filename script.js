...
(() => {
  ...
  // === التعديل الذكي لحل مشكلة النزول للأسفل ===
  document.querySelectorAll("[data-scroll]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      // تأخير عملية التمرير (Scroll) لمدة 380 جزء من الثانية
      // لضمان استقرار طول الصفحة بعد فتح/إغلاق الأقسام
      setTimeout(() => {
        try {
          const offset = typeof getStickyOffset === "function" ? getStickyOffset() : 0;
          // حساب المكان الدقيق للقسم بعد استقرار الصفحة (مع ترك 12 بكسل مسافة إضافية من الأعلى لراحة العين)
          const y = (el.getBoundingClientRect().top + (window.pageYOffset || 0)) - offset - 12;
  ...
  offerBuyButtons.forEach((btn)=>{
    btn.addEventListener("click", (e)=>{
      ...
      if (orderPanel){
        ...
        // تأخير ذكي للنزول لقسم الطلب بعد الضغط على "شراء"
        setTimeout(() => {
          try {
            const offset = typeof getStickyOffset === "function" ? getStickyOffset() : 0;
            const y = (orderPanel.getBoundingClientRect().top + (window.pageYOffset || 0)) - offset - 12;
            window.scrollTo({ top: Math.max(0, Math.round(y)), behavior: "smooth" });
          } catch (_e) {}
        }, 380);
      }
