(() => {
  ...
  const dict = {
    ...
    phComment: { ar: "اكتب تعليقك...", en: "Write your comment...", he: "כתוב تגובה..." },
    ...
    replyPlaceholder: { ar: "اكتب ردك...", en: "Write your reply...", he: "כתוב تגובה..." },
    replyNamePlaceholder: { ar: "الاسم", en: "Name", he: "شם" },
  ...
  const seedReviews = [
    ...
    { 
      ...
      replies: [
        ...
        { name: { ar: "بندر", en: "Bandar", he: "بנדר" }, text: { ar: "التجفيف أهم خطوة، بدونه ما يمتصه الجلد صح.", en: "Drying is the most important step.", he: "ייבוש הוא השלב החשוב ביותר." } }
      ]
    },
    { 
...
(() => {
  ...
  // حساب المساحة التي يجب خصمها عند النزول (شريط اللغات + شريط الأزرار)
  function getStickyOffset(){
    let h = 0;
    const langbar = document.querySelector(".langbar");
    if(langbar) h += langbar.getBoundingClientRect().height || 0;
    const actionRow = document.querySelector(".actionRow");
    if(actionRow) h += actionRow.getBoundingClientRect().height || 0;
    return h;
  }
    ...
    btn.addEventListener("click", (e) => {
      ...
      setTimeout(() => {
        try {
          ...
          // تم التصحيح هنا لضمان النزول للقسم نفسه وخصم مساحة الأشرطة العلوية
          const y = (el.getBoundingClientRect().top + (window.pageYOffset || window.scrollY)) - offset - 15;
  ...
  if (reviewForm && rfName && rfComment) {
    ...
    function enableForm() {
      rfName.disabled = false; rfName.style.opacity = "1"; rfName.style.cursor = "text"; rfName.style.pointerEvents = "auto";
      rfComment.disabled = false; rfComment.style.opacity = "1"; rfComment.style.cursor = "text"; rfComment.style.pointerEvents = "auto";
      if (rfSubmit) { rfSubmit.disabled = false; rfSubmit.style.opacity = "1"; rfSubmit.style.cursor = "pointer"; rfSubmit.style.pointerEvents = "auto"; }
      const lbl = document.getElementById("rfRateLabel");
      if (lbl) lbl.style.color = "#10b981"; 
    }

    function disableForm() {
      rfName.disabled = true; rfName.style.opacity = "0.5"; rfName.style.cursor = "not-allowed"; rfName.style.pointerEvents = "none";
      rfComment.disabled = true; rfComment.style.opacity = "0.5"; rfComment.style.cursor = "not-allowed"; rfComment.style.pointerEvents = "none";
      if (rfSubmit) { rfSubmit.disabled = true; rfSubmit.style.opacity = "0.5"; rfSubmit.style.cursor = "not-allowed"; rfSubmit.style.pointerEvents = "none"; }
      const lbl = document.getElementById("rfRateLabel");
    ...
    reviewForm.addEventListener("submit", (e) => {
      ...
      setTimeout(() => {
        ...
        if (rfSubmit) { rfSubmit.classList.remove("isLoading"); }
      }, 650);
    });
  }
  ...
  const prices = { "1": 130, "2": 200, "3": 250 };

  const updateTotal = () => {
    if (!qtySelect || !totalPrice) return;
    const v = qtySelect.value || "1";
    const curr = (dict.currency && dict.currency[currentLang]) || dict.currency.ar;
    totalPrice.textContent = `${prices[v] ?? 130} ${curr}`;
  };
    ...
    btn.addEventListener("click", (e)=>{
      ...
      if (orderPanel){
        ...
        setTimeout(() => {
          try {
            const offset = typeof getStickyOffset === "function" ? getStickyOffset() : 0;
            // تم التصحيح هنا لضمان النزول لنموذج الطلب بدقة وخصم مساحة الأشرطة
            const y = (orderPanel.getBoundingClientRect().top + (window.pageYOffset || window.scrollY)) - offset - 15;
            window.scrollTo({ top: Math.max(0, Math.round(y)), behavior: "smooth" });
          } catch (_e) {}
        }, 380);
      }
  ...
  const popupCities = {
    ar: ["دبي", "أبوظبي", "الشارقة", "الرياض", "جدة", "الدمام", "مسقط", "صلالة", "الدوحة", "المنامة", "الكويت"],
    en: ["Dubai", "Abu Dhabi", "Sharjah", "Riyadh", "Jeddah", "Dammam", "Muscat", "Salalah", "Doha", "Manama", "Kuwait"],
    he: ["דובאי", "אבו דאבי", "שארג'ה", "ריאד", "ג'דה", "דמאם", "מוסקט", "סלאלה", "דוחה", "מנאמה", "כווית"]
  };
  const popupItems = {
