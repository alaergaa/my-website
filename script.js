...
alreadyRated: { ar: "تم استلام تقييمك مسبقًا.", en: "You already rated.", he: "כבר דירגת." },
  ...
  reviewsCount: { ar: "عدد التقييمات: {n}", en: "Total ratings: {n}", he: "מספר דירוגים: {n}" },
  ...
  langLoading: { ar: "جاري تغيير اللغة...", en: "Switching language...", he: "מחליף שפה..." },
};
  ...
  function renderReviews(lang, reset){
    ...
    const step = (reviewsShown === 0 ? REVIEWS_INITIAL : REVIEWS_PAGE);
    const nextCount = Math.min(all.length, reviewsShown + step);
  ...
  function updateReviewsCount(lang){
    const el = document.getElementById("reviewsCount");
    if(!el) return;
    const total = (userReviews.length + seedReviews.length);
    const tpl = dict.reviewsCount && dict.reviewsCount[lang] ? dict.reviewsCount[lang] : ("عدد التقييمات: {n}");
    el.textContent = formatTpl(tpl, {n: total});
  }

  function showLangToast(lang){
    const toast = document.getElementById("langToast");
    if(!toast) return;
    // Update message for selected language
    const msgEl = toast.querySelector("[data-i18n='langLoading']");
    if(msgEl && dict.langLoading && dict.langLoading[lang]) msgEl.textContent = dict.langLoading[lang];
  ...
  const setLang = (lang, opts = {}) => {
    ...
    if(opts.showToast) showLangToast(lang);
  ...
  document.querySelectorAll(".langchip").forEach(btn => {
    btn.addEventListener("click", () => setLang(btn.dataset.lang, { showToast: true }));
  });
