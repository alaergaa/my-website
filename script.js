...
(() => {
  ...
  document.addEventListener('click', (e) => {
    if(e.target && e.target.classList.contains('faq-q')) {
      ...
      if(parent) {
        ...
        // Update max-height dynamically if needed
  ...
  const popupItems = ["عبوتين", "عبوة واحدة", "3 عبوات", "عبوتين", "عبوتين"]; // تكرار "عبوتين" لزيادة احتمالية ظهور العرض الأكثر طلباً
  ...
  function triggerLivePopup() {
    ...
    const min = Math.floor(Math.random() * 12) + 1; // رقم عشوائي بين 1 و 12 دقيقة
  ...
  // تشغيل الإشعار الأول بعد 4 ثواني من فتح الصفحة
  setTimeout(triggerLivePopup, 4000);

  // تشغيل الإشعارات بشكل متكرر وعشوائي كل 15 إلى 25 ثانية
  setInterval(() => {
    triggerLivePopup();
