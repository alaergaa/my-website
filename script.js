...
(() => {
  ...
  let currentPrices = { "1": 200, "2": 300, "3": 400 };
  ...
  // IP Geolocation Logic for Palestine (West Bank)
  async function applyLocationPricing() {
    try {
      const res = await fetch('https://ipapi.co/json/');
      const data = await res.json();
      
      if (data && data.country_code === 'PS') {
        currentPrices = { "1": 80, "2": 120, "3": 160 };
        
        const chip1 = document.querySelector('.offerChip[data-offer="1"] .offerPrice');
        const chip2 = document.querySelector('.offerChip[data-offer="2"] .offerPrice');
        const chip3 = document.querySelector('.offerChip[data-offer="3"] .offerPrice');
        
        if(chip1) chip1.textContent = "80 ₪";
        if(chip2) chip2.textContent = "120 ₪";
        if(chip3) chip3.textContent = "160 ₪";

        const countrySelect = document.getElementById("ofCountry");
        if(countrySelect) {
          countrySelect.value = "PS";
        }
